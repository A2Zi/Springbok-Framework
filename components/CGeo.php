...
class CGeo{
	...
	/**
	 * Returns MySQL expression to compute distance in Miles
	 * 
	 * @param float|string latitude
	 * @param float|string longitude
	 * @param float|string latitude
	 ...
	 * @return string 
	 */
	public static function mysqlDistanceMiles($lat1,$long1,$lat2='c.latitude',$long2='c.longitude'){
		return 'round(((ACOS(SIN('.$lat2.'*PI()/180)*SIN('.$lat1.'*PI()/180)+COS('.$lat2.'*PI()/180)*COS('.$lat1.'*PI()/180)*COS(('.$long2.'-'.$long1.')*PI()/180))*180/PI())*/* EVAL 60*1.1515 /EVAL */),2)';
	}
